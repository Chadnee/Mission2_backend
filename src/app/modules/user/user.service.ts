...
const createStudentUserIntoDB = async (
  file: any,
  password: string,
...
const createFacultyUserIntoDB = async (
  file: any,
  passWord: string,
  ...
  try {
    ...
    if (Object.keys(newUser).length) {
      ...
      if (!newFaculty.length) {
        throw new AppError(
          status.BAD_REQUEST,
          'New faculty creation is failed',
        );
      }
...
const createAdminUserIntoDB = async (
  file: any,
  password: string,
  adminData: TAdmin,
) => {
